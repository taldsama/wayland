...
vi.mock('@arco-design/web-react', () => {
  ...
  return {
    Button: ({ children, onClick, disabled, ['data-testid']: testid }: Record<string, unknown>) => (
      <button type='button' disabled={disabled as boolean} data-testid={testid as string} onClick={onClick as () => void}>
...
describe('<ConciergeConfigCard>', () => {
  ...
  it('set_default_model: Accept calls confirmProposal with action accept (no secret)', () => {
    render(<ConciergeConfigCard message={msg({ kind: 'set_default_model', engine: 'wcore', modelId: 'm', useModel: 'u', label: 'X', status: 'pending' })} />);
    fireEvent.click(acceptBtn());
    expect(confirmSpy).toHaveBeenCalledWith(expect.objectContaining({ conversationId: 'c1', msgId: 'm1', action: 'accept', secret: undefined }));
  });

  it('provider_connect: Accept disabled until a key is typed, then sends it as secret', () => {
    render(<ConciergeConfigCard message={msg({ kind: 'provider_connect', providerId: 'openai', label: 'OpenAI', status: 'pending' })} />);
  ...
  it('parseError disables Accept', () => {
    render(<ConciergeConfigCard message={msg({ kind: 'set_default_model', engine: 'wcore', modelId: 'm', useModel: 'u', label: 'X', status: 'pending', parseError: true })} />);
    expect(acceptBtn()).toBeDisabled();
  });

  it('add_mcp env values render masked (raw value absent from the DOM)', () => {
    render(<ConciergeConfigCard message={msg({ kind: 'add_mcp', name: 'fs', command: 'npx', args: ['-y', 'srv'], env: { API_KEY: 'supersecret1234' }, status: 'pending' })} />);
  ...
  it('Cancel calls confirmProposal with action cancel', () => {
    render(<ConciergeConfigCard message={msg({ kind: 'edit_assistant', assistantId: 'a', label: 'A', rules: 'r', status: 'pending' })} />);
