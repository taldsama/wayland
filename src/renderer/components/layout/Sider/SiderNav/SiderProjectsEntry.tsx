...
const SiderProjectsEntry: React.FC<SiderProjectsEntryProps> = ({
  ...
  const label = t('projects.siderEntry', { defaultValue: 'Projects' });
  ...
  if (collapsed) {
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        ...
        >
          <FolderKanban size={16} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-20px h-20px flex items-center justify-center shrink-0'>
          <FolderKanban size={16} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
