...
const SiderGoalEntry: React.FC<SiderGoalEntryProps> = ({
  isMobile,
  isActive,
  collapsed,
  siderTooltipProps,
  ...
  const label = t('goal.siderEntry', { defaultValue: 'Goal YOLO' });
  ...
  if (collapsed) {
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        ...
        >
          <Target size={16} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-20px h-20px flex items-center justify-center shrink-0'>
          <Target size={16} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
