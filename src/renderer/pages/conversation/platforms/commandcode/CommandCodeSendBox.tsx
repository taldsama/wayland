...
interface CommandCodeDraftData {
  _type: 'command-code';
...
const useCommandCodeSendBoxDraft = getSendBoxDraftHook('command-code', {
  _type: 'command-code',
  atPath: [],
  content: '',
  uploadFile: [],
...
const CommandCodeSendBox: React.FC<{ conversation_id: string }> = ({ conversation_id }) => {
  ...
  const { data: draftData, mutate: mutateDraft } = useCommandCodeSendBoxDraft(conversation_id);
  ...
  const setAtPath = useCallback(
    (val: Array<string | FileOrFolderItem>) => {
      mutateDraft((prev) => ({ ...(prev as CommandCodeDraftData), atPath: val }));
    },
    [mutateDraft]
  );
  ...
  const setContent = useCallback(
    (val: string) => {
      mutateDraft((prev) => ({ ...(prev as CommandCodeDraftData), content: val }));
    },
    [mutateDraft]
  );
  ...
  useAddEventListener('command-code.selected.file', (items: Array<string | FileOrFolderItem>) => {
    setTimeout(() => {
      setAtPath(items);
    }, 10);
  });

  useAddEventListener('command-code.selected.file.append', (items: Array<string | FileOrFolderItem>) => {
    setTimeout(() => {
  ...
  const executeCommand = useCallback(
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        assertBridgeSuccess(result, 'Failed to send message to CommandCode');
  ...
  const onSendHandler = async (message: string) => {
    emitter.emit('command-code.selected.file.clear');
  ...
  // Handle initial message from guid page - command-code is stateless, send immediately
  useEffect(() => {
    if (!conversation_id) return;

    const storageKey = `command-code_initial_message_${conversation_id}`;
    const processedKey = `command-code_initial_processed_${conversation_id}`;
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        assertBridgeSuccess(result, 'Failed to send initial message to CommandCode');
        emitter.emit('chat.history.refresh');
    ...
    <div className='max-w-800px w-full mx-auto flex flex-col mt-auto mb-16px'>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(nextSelectedItems) => {
          emitter.emit('command-code.selected.file', nextSelectedItems);
        ...
        placeholder={
          aiProcessing
            ...
            : t('acp.sendbox.placeholder', {
                backend: 'CommandCode',
                defaultValue: `Send message to CommandCode...`,
              })
        }
        onStop={handleStop}
                    ...
                    return (
                      <Tag
                        ...
                        onClose={() => {
                          ...
                          emitter.emit('command-code.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
...
export default CommandCodeSendBox;
