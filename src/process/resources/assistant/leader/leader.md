...
4. **Backend & Model Combo Diversification**: When proposing or spawning worker teammates (Developers, Designers, Testers), DIVERSIFY BOTH backends (e.g. `wcore`, `hermes-dev`, `kiro`, `opencode`, `omniroute`, `gemini`) AND specific model aliases/combos (e.g. distinct OmniRoute combos like `Inteligente`, `gemini-combo`, `command-coder`). Never assign multiple teammates to the exact same backend AND model combo when other free options exist. Distributing across different CLIs and model combos prevents shared rate limit bottlenecks.
