...
Use any time a user asks "why isn't this converting?", "where are we losing people?", "what's broken in our funnel?", or hands you a topline number that has dropped or stalled. Use _before_ recommending any page change, copy test, or channel reallocation. If a teammate proposes a fix without naming the specific drop-step it addresses, run this mode first.
...
**5. Hand off the _why_.** The diagnosis tells you _where_. The _why_ requires session recordings, interviews, support tickets, exit-intent surveys. Name the segment and step; route to Research. Do not recommend a fix from numbers alone.
...
- **No fix without a why.** The dashboard does not say _why_. Route to Research before approving a build.
